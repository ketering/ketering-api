...
class StatusesController extends Controller
{
    ...
    {
        //
        return redirect()->route('orders.statuses.show', $status);
    }
    ...
    public function update(Request $request, Status $status)
    {
        ...
        if ($validator->fails()) {
            return redirect(route('orders.statuses.show', $status) . '#settings')->withErrors($validator->errors());
        }
     ...
     */
    public function destroy(int $name)
    {
        //
        $status = Status::findOrFail($name);
        try {
            $status->delete();
        } catch (\Exception $e) {
